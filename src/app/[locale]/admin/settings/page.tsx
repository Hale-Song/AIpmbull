...
import { PageHeader, BilingualField, AdminInput } from "@/components/admin";
      ...
      <div className="space-y-6">
        <div className="rounded-xl border border-slate-800 bg-slate-900 p-6">
          ...
          <div className="space-y-4">
            <BilingualField
              label={isZh ? "站点名称" : "Site Name"}
              valueZh={settings.siteName}
              valueEn={settings.siteNameEn}
              onChangeZh={(v) => update("siteName", v)}
              onChangeEn={(v) => update("siteNameEn", v)}
            />
            <AdminInput
              label={isZh ? "Logo 文字" : "Logo Text"}
              value={settings.logo}
              onChange={(v) => update("logo", v)}
            />
            <BilingualField
              label={isZh ? "站点描述" : "Description"}
              valueZh={settings.descriptionZh}
              valueEn={settings.descriptionEn}
        ...
        <div className="rounded-xl border border-slate-800 bg-slate-900 p-6">
          <h3 className="mb-4 text-sm font-semibold uppercase text-slate-400">{isZh ? "GitHub 图片存储" : "GitHub Image Storage"}</h3>
          <p className="mb-4 text-xs text-slate-500">{isZh ? "配置 GitHub 仓库信息，用于持久化存储素材库图片。需要 Personal Access Token (repo 权限)。" : "Configure GitHub repo for persistent image storage. Requires a Personal Access Token with repo scope."}</p>
          <div className="space-y-4">
            <AdminInput
              label={isZh ? "Personal Access Token" : "Personal Access Token"}
              type="password"
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              <AdminInput
                label={isZh ? "仓库 (owner/repo)" : "Repository (owner/repo)"}
                value={settings.githubRepo}
                onChange={(v) => update("githubRepo", v)}
                placeholder="Hale-Song/AIpmbull"
              />
              <AdminInput
            ...
            <AdminInput
              label={isZh ? "图片存储路径" : "Image Path"}
              value={settings.githubImagePath}
              onChange={(v) => update("githubImagePath", v)}
