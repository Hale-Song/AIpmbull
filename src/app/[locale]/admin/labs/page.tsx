...
export default function AdminLabsPage({ params: { locale } }: { params: { locale: string } }) {
  ...
  const handleDelete = (id: string) => { store.delete("labs", id); setDeleteConfirm(null); load(); syncAllToApi(); };
  const togglePublish = (id: string, p: boolean) => { store.update<LabTemplate>("labs", id, { published: !p }); load(); syncAllToApi(); };
